...
async function verifyFilters() {
    ...
    try {
        ...
        if (!priorityRes.ok) {
            ...
            const invalidPriority = priorityData.filter((c: any) => c.priority !== 'high');
            if (invalidPriority.length > 0) {
                console.error("❌ FAILED: Found complaints with non-high priority:", invalidPriority.map((c: any) => c.priority));
            } else {
        ...
        const sampleState = allData.find((c: any) => c.location?.stateName)?.location?.stateName;
        ...
        if (!sampleState) {
            ...
            if (!stateRes.ok) {
                ...
                const invalidState = stateData.filter((c: any) => c.location?.stateName !== sampleState);
                if (invalidState.length > 0) {
                    console.error(`❌ FAILED: Found complaints with non-${sampleState} state:`, invalidState.map((c: any) => c.location?.stateName));
                } else {
